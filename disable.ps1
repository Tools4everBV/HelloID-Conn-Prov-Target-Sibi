...
try {
    ...
    switch ($action) {
        'DisableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling Sibi account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatDisableParams
            } else {
                Write-Information "[DryRun] Disable Sibi account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable Sibi account. Error: $($_.Exception.Message)"
